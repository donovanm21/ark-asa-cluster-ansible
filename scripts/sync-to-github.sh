...
#   git remote add github git@github.com:OWNER/ark-asa-cluster-ansible.git
#
# What it does:
...
git remote | grep -qx "$REMOTE" \
    || die "remote '$REMOTE' not configured. add it: git remote add $REMOTE git@github.com:OWNER/ark-asa-cluster-ansible.git"
