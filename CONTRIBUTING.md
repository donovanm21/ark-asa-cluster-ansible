...
Thanks for wanting to improve `ark-asa-cluster-ansible`. This is a small project — contribution flow is deliberately lightweight.
...
1. Open an issue describing what you want to change, especially if it touches:
   ...
   - The container image, mount layout, or `asa_user` defaults
...
```sh
# On the target host
cd /root/ark-asa-cluster-ansible       # (or wherever you cloned)
git pull
...
- Variable names: `snake_case`. Project-wide vars are `asa_*`.
